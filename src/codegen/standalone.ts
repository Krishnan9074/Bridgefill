...
import type {
  ...
  StandaloneGenerateRequest,
  StandaloneGenerateResponse,
} from "../types.js";
...
export class StandaloneRequestError extends Error {
  ...
  constructor(message: string, statusCode = 400, machineCode = "VALIDATION_ERROR") {
    super(message);
...
export interface StandaloneAuthContext {
  orgId: string;
  orgName: string;
  role: OrgClaims["role"] | "api_key";
  authType: "bearer" | "api_key";
}

export interface ResolvedRegistrySchema {
  serviceId: string;
  serviceName: string;
  entry: RegistrySchemaRecord;
}

function cloneContext(consumerContext: ConsumerContext | undefined, options: GenerationOptions | undefined): ConsumerContext {
  return {
    ...(consumerContext ?? {}),
...
export function resolveRegistrySchema(serviceReference: string, version = "latest"): ResolvedRegistrySchema {
  if (!serviceReference) {
...
export async function generateFromRegistry({
  ...
  orgId: string;
}): Promise<StandaloneGenerateResponse> {
...
export function validateGeneratedFiles({
  ...
  entry: RegistrySchemaRecord;
  files: GeneratedFile[] | Array<{ filename: string; content: string }>;
}): {
  passed: boolean;
  issue_count: number;
  issues: Array<{ severity: string; endpoint: string | null; message: string }>;
...
export function shouldRunAsync(entry: RegistrySchemaRecord, options?: GenerationOptions, consumerContext?: ConsumerContext): boolean {
...
export function resolveStandaloneAuth({
  bearerToken,
  rawApiKey,
}: {
  bearerToken?: string | null;
  rawApiKey?: string | null;
}): StandaloneAuthContext {
  if (bearerToken) {
...
export function buildJobRecord(orgId: string, request: StandaloneGenerateRequest): GenerateJobRecord {
  return {
