...
import {
  ...
  getRegistryEntry,
  getSchemaHistory,
  listRegistry as listRegistryEntries,
  publishToRegistry,
...
import type { CodeSample, ConsumerContext, GeneratedOutput, NormalizedSchema, OrgClaims, RawSchemaContract, RegistrySchemaRecord, ServiceEntry, SessionRecord } from "../types.js";
...
function toolFailure(toolName: string, orgId: string, error: unknown): never {
  const err = error as { code?: number; message?: string };
  auditTool.failed(orgId, toolName, err.code ?? -32000, err.message ?? "Internal error");
  throw error;
}

async function withAuthedTool<T>(args: { org_token?: string; session_id?: string | null }, toolName: string, executor: (claims: OrgClaims) => Promise<T>): Promise<T> {
...
function assertSessionActive(session: SessionRecord): void {
  if (session.status !== "active") {
    throw new ValidationError("Session must be active");
  }
}

function normaliseSchema(raw: RawSchemaContract): NormalizedSchema {
  return {
...
function registrySummary(entry: RegistrySchemaRecord): Record<string, unknown> {
  return {
...
export async function ping({ echo }: { echo?: unknown } = {}): Promise<Record<string, unknown>> {
  return {
    status: "ok",
...
export async function joinSession(args: {
  ...
  return withAuthedTool(args, "join_session", async (claims) => {
    ...
    if (session.status === "active") {
      auditSession.activated(
        session.id,
        session.participants.provider?.orgId ?? null,
        session.participants.consumer?.orgId ?? null,
      );
    }
...
export async function publishSchema(args: {
  org_token?: string;
  session_id: string;
  schema: RawSchemaContract;
}): Promise<Record<string, unknown>> {
  return withAuthedTool(args, "publish_schema", async (claims) => {
    const session = getSession(args.session_id, claims.orgId);
    assertSessionActive(session);

    const schema = args.schema ?? ({} as RawSchemaContract);
    if (!schema.base_url) {
