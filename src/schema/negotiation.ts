import type { Change, ConflictReport, EnrichedChange, NegotiationResult, NormalizedSchema, SchemaDiff, VersionRecord } from "../types.js";

type DiffSchema = {
  base_url: string;
  auth: {
    type?: NormalizedSchema["auth"]["type"];
    key_name?: string | null;
    location?: NormalizedSchema["auth"]["location"];
  };
  endpoints: Array<{
    path: string;
    method: string;
...
type DiffParam = NonNullable<DiffSchema["endpoints"][number]["all_params"]>[number];

function parseEndpointKey(endpoint: Pick<DiffSchema["endpoints"][number], "method" | "path">): string {
  return `${endpoint.method}:${endpoint.path}`;
}

function groupParams(params: DiffParam[] = []): {
  required: DiffParam[];
  optional: DiffParam[];
  byName: Map<string, DiffParam>;
} {
  const required: DiffParam[] = [];
  const optional: DiffParam[] = [];
  const byName = new Map<string, DiffParam>();
...
function endpointPathFromChange(changePath: string): string | null {
...
function makeChange(severity: Change["severity"], path: string, change: string, from: unknown, to: unknown, message: string): Change {
  return { severity, path, change, from, to, message };
}

function determineBump(counts: { breaking: number; warning: number; additive: number }): SchemaDiff["suggestedVersionBump"] {
...
export function diff(prev: DiffSchema, next: DiffSchema): SchemaDiff {
  ...
  for (const key of ["requests_per_second", "requests_per_day"] as const) {
...
export function detectConflicts(schemaDiff: SchemaDiff, consumerContext: { endpoints_needed?: string[]; language?: string } = {}): ConflictReport {
  ...
  const enrich = (change: Change): EnrichedChange => ({
    ...change,
...
export function negotiate({ publishedSchema, previousSchema, consumerContext = {} }: {
  ...
  previousSchema: NormalizedSchema | null;
  consumerContext?: { endpoints_needed?: string[]; language?: string };
}): NegotiationResult {
  ...
  const blockedEndpoints = Array.from(new Set(conflicts.conflicts.map((change) => endpointPathFromChange(change.path)).filter((value): value is string => !!value)));
...
function incrementVersion(version: string, bump: SchemaDiff["suggestedVersionBump"]): string {
...
export function bumpVersion(history: VersionRecord[], newSchema: NormalizedSchema, schemaDiff: SchemaDiff | null): {
  version: string;
