...
export async function createPostgresStores({ pgUrl }) {
    ...
    for (const row of servicesResult.rows) {
        const service = row.data;
        servicesById.set(service.id, service);
    }
    for (const row of sessionsResult.rows) {
        const session = { ...row.data, _expiryTimer: null };
        sessionsById.set(session.id, session);
        sessionIdsByService.set(session.serviceId, session.id);
    }
    for (const row of keysResult.rows) {
        const key = { ...row.data, hash: row.hash, _revokeTimer: null };
    ...
    for (const row of registryResult.rows) {
        ...
        const orgName = config.orgs[row.org_id]?.name ?? String(row.org_id);
        const record = {
    ...
    return {
        sessions: {
            ...
            async set(id, data) {
                ...
                await pool.query(`INSERT INTO sessions (id, service_id, status, data, created_at, updated_at)
           VALUES ($1, $2, $3, $4::jsonb, $5::timestamptz, NOW())
           ON CONFLICT (id) DO UPDATE SET
             service_id = EXCLUDED.service_id,
             status = EXCLUDED.status,
             data = EXCLUDED.data,
             updated_at = NOW()`, [id, data.serviceId, data.status, JSON.stringify(withoutTimer(data)), data.createdAt]);
            },
            async del(id) {
        ...
        services: {
            ...
            async set(id, data) {
                ...
                await pool.query(`INSERT INTO services (id, provider_org_id, name, data, created_at)
           VALUES ($1, $2, $3, $4::jsonb, $5::timestamptz)
           ON CONFLICT (id) DO UPDATE SET
             provider_org_id = EXCLUDED.provider_org_id,
             name = EXCLUDED.name,
             data = EXCLUDED.data`, [id, data.providerOrgId, data.name, JSON.stringify(data), data.registeredAt]);
            },
            list() {
        ...
        keys: {
            ...
            async save(record) {
                ...
                await pool.query(`INSERT INTO api_keys (key_id, org_id, hash, status, label, expires_at, last_used_at, data)
        ...
        audit: {
            async append(entry) {
                ...
                await pool.query(`INSERT INTO audit_log (seq, ts, category, event, org_id, session_id, data)
           VALUES ($1, $2::timestamptz, $3, $4, $5, $6, $7::jsonb)`, [
                    entry.seq,
                    entry.ts,
                    entry.category,
                    entry.event,
                    entry.orgId ?? entry.initiatorOrgId ?? null,
        ...
        registry: {
            async save(record) {
                ...
                await pool.query(`INSERT INTO registry_schemas (registry_id, service_id, org_id, version, schema_data, code_samples, changelog, tags, is_latest, published_at)
        ...
        jobs: {
            ...
            async set(jobId, data) {
                ...
                await pool.query(`INSERT INTO generation_jobs (job_id, status, org_id, request_data, result_data, error, created_at, completed_at)
