...
async function runCheck(message: string, fn: () => Promise<void>): Promise<number> {
  try {
    await fn();
    pass(message);
    return 1;
...
async function issueToken(server: Awaited<ReturnType<typeof import("../src/server.js").buildServer>>, payload: Record<string, unknown>): Promise<Record<string, unknown>> {
...
async function callTool(server: Awaited<ReturnType<typeof import("../src/server.js").buildServer>>, id: number, name: string, args: Record<string, unknown>): Promise<Record<string, unknown>> {
  const response = await server.inject({
    method: "POST",
    url: "/mcp",
    payload: {
...
async function main(): Promise<void> {
  ...
  passed += await runCheck("POST /mcp initialize returns capabilities with tools key", async () => {
    ...
    const body = response.json() as { result?: { capabilities?: { tools?: unknown } } };
    assert(response.statusCode === 200, `Expected 200, received ${response.statusCode}`);
  ...
  passed += await runCheck('tools/call ping returns { status: "ok" }', async () => {
    ...
    const toolPayload = JSON.parse(body.result.content[0].text) as { status: string };
  ...
  let asyncJobId = "";
  let standaloneGeneratedFiles: Array<{ filename: string; content: string }> = [];
  ...
  passed += await runCheck("Consumer API key can also mint a scoped consumer token", async () => {
    ...
    consumerToken = body.token as string;
  ...
  passed += await runCheck("Provider can register a service with the scoped token", async () => {
    ...
    const toolPayload = JSON.parse((response.result as { content: Array<{ text: string }> }).content[0].text) as { service_id: string };
  ...
  passed += await runCheck("Provider can join a session for the new service", async () => {
    ...
    const toolPayload = JSON.parse((response.result as { content: Array<{ text: string }> }).content[0].text) as { session_id: string; status: string };
  ...
  passed += await runCheck("Consumer can join the same session and activate it", async () => {
    ...
    const toolPayload = JSON.parse((response.result as { content: Array<{ text: string }> }).content[0].text) as { status: string };
  ...
  passed += await runCheck("Provider token cannot call discover_schema", async () => {
    ...
    assert((response.error as { code?: number }).code === -32001, `Expected -32001, received ${(response.error as { code?: number }).code}`);
  });
  ...
  passed += await runCheck("POST /auth/keys creates a new API key and exposes it once", async () => {
    ...
    const body = response.json() as { key_id: string; raw_key: string };
  ...
  passed += await runCheck("Old rotated key is rejected after the grace period", async () => {
    ...
    const createdKeyBody = createdKeyResponse.json() as { raw_key: string; key_id: string };
  ...
  passed += await runCheck('Publish schema v1 gets version "1.0.0"', async () => {
    ...
    const newServiceId = (JSON.parse((registerResponse.result as { content: Array<{ text: string }> }).content[0].text) as { service_id: string }).service_id;

    const providerJoin = await callTool(server, 21, "join_session", {
      org_token: providerToken,
      service_id: newServiceId,
    });
    schemaSessionId = (JSON.parse((providerJoin.result as { content: Array<{ text: string }> }).content[0].text) as { session_id: string }).session_id;
    ...
    const payload = JSON.parse((publishResponse.result as { content: Array<{ text: string }> }).content[0].text) as { version: string };
  ...
  passed += await runCheck('Publish schema v2 with removed endpoint and new required param gets "2.0.0" with 2 breaking changes', async () => {
    ...
    const payload = JSON.parse((publishResponse.result as { content: Array<{ text: string }> }).content[0].text) as {
      version: string;
      diff_summary: { breaking: number; warnings: number };
    };
  ...
  passed += await runCheck('discover_schema returns version history with 2 entries', async () => {
    ...
    const payload = JSON.parse((response.result as { content: Array<{ text: string }> }).content[0].text) as { version_history: unknown[] };
  ...
  passed += await runCheck('Publish schema with only a new optional param gets "1.1.0"', async () => {
    ...
    const serviceId2 = (JSON.parse((registerResponse.result as { content: Array<{ text: string }> }).content[0].text) as { service_id: string }).service_id;
    const providerJoin = await callTool(server, 27, "join_session", {
      org_token: providerToken,
      service_id: serviceId2,
    });
    schemaSessionId2 = (JSON.parse((providerJoin.result as { content: Array<{ text: string }> }).content[0].text) as { session_id: string }).session_id;
      ...
      endpoints: [
        {
          ...
          method: "GET" as const,
          summary: "",
          all_params: [{ name: "query", in: "query" as const, required: true, schema: { type: "string" }, description: "" }],
        },
        { path: "/v1/nearby", method: "GET" as const, summary: "", all_params: [] },
      ],
    };
      ...
      endpoints: [
        {
          ...
          summary: "",
          all_params: [
            { name: "query", in: "query" as const, required: true, schema: { type: "string" }, description: "" },
            { name: "region", in: "query" as const, required: true, schema: { type: "string" }, description: "" },
          ],
        },
      ],
    };
  ...
  passed += await runCheck("provide_code_sample stores provider code for later generation", async () => {
    ...
    const phase4ServiceId = (JSON.parse((registerResponse.result as { content: Array<{ text: string }> }).content[0].text) as { service_id: string }).service_id;

    const providerJoin = await callTool(server, 33, "join_session", {
      org_token: providerToken,
      service_id: phase4ServiceId,
    });
    phase4SessionId = (JSON.parse((providerJoin.result as { content: Array<{ text: string }> }).content[0].text) as { session_id: string }).session_id;
    ...
    const payload = JSON.parse((sampleResponse.result as { content: Array<{ text: string }> }).content[0].text) as { total_samples: number };
  ...
  passed += await runCheck("generate_integration without an API key returns fallback/provider_sample files", async () => {
    ...
    const payload = JSON.parse((response.result as { content: Array<{ text: string }> }).content[0].text) as {
      source: string;
      files: Array<{ source: string; content: string }>;
    };
  ...
  passed += await runCheck("validate_integration passes for generated fallback/provider output", async () => {
    ...
    const sessionPayload = JSON.parse((generatedResponse.result as { content: Array<{ text: string }> }).content[0].text) as {
      generated_code: {
    ...
    const validPayload = JSON.parse((validResponse.result as { content: Array<{ text: string }> }).content[0].text) as { passed: boolean };
  ...
  passed += await runCheck("validate_integration detects missing required params", async () => {
    ...
    const invalidPayload = JSON.parse((invalidResponse.result as { content: Array<{ text: string }> }).content[0].text) as {
      passed: boolean;
      issue_count: number;
    };
  ...
  passed += await runCheck("emit_message stores messages visible in get_session_status", async () => {
    ...
    const messagePayload = JSON.parse((messageResponse.result as { content: Array<{ text: string }> }).content[0].text) as { session_message_count: number };
    ...
    const statusPayload = JSON.parse((statusResponse.result as { content: Array<{ text: string }> }).content[0].text) as {
  ...
  passed += await runCheck("generate_integration with an LLM API key uses the real configured LLM when available", async () => {
    ...
    const llmServiceId = (JSON.parse((registerResponse.result as { content: Array<{ text: string }> }).content[0].text) as { service_id: string }).service_id;

    const providerJoin = await callTool(server, 44, "join_session", {
      org_token: providerToken,
      service_id: llmServiceId,
    });
    llmSessionId = (JSON.parse((providerJoin.result as { content: Array<{ text: string }> }).content[0].text) as { session_id: string }).session_id;
    ...
    const payload = JSON.parse((generateResponse.result as { content: Array<{ text: string }> }).content[0].text) as {
      source: string;
      model: string | null;
      files: Array<{ source: string; content: string }>;
    };
  ...
  passed += await runCheck("POST /generate with more than 5 endpoints returns 202 and a job id", async () => {
    ...
    largeRegistryServiceId = (JSON.parse((registerResponse.result as { content: Array<{ text: string }> }).content[0].text) as { service_id: string }).service_id;
    ...
    const payload = response.json() as { job_id: string; status: string; poll: string };
  ...
  passed += await runCheck("GET /ready returns 200 with ready:true for memory backend", async () => {
    ...
    const body = response.json() as { ready: boolean; backend?: string; reason?: string };
    if ((process.env.STORE_BACKEND ?? "memory") === "memory") {
      assert(response.statusCode === 200, `Expected 200, received ${response.statusCode}`);
  ...
  passed += await runCheck("Security headers are set (x-frame-options and x-content-type-options)", async () => {
    ...
    assert(
      typeof headers["x-frame-options"] === "string" || typeof headers["x-content-type-options"] === "string",
      `Expected security headers, received: x-frame-options=${headers["x-frame-options"]}, x-content-type-options=${headers["x-content-type-options"]}`,
    );
  });
  ...
  passed += await runCheck("Rate-limit headers are present on rate-limited routes", async () => {
    ...
    const headers = response.headers;
    assert(
      typeof headers["x-ratelimit-limit"] === "string" || typeof headers["ratelimit-limit"] === "string",
      `Expected rate-limit headers, received keys: ${Object.keys(headers).join(", ")}`,
    );
  });
